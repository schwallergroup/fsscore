...
def filter_big_scc(g, edges_to_be_removed):
    ...
    sub_graphs = filter(
        lambda scc: scc.number_of_nodes() >= 2,
...
def scc_nodes_edges(g):
    ...
    print(
        "# nodes in biggest scc: %d, # edges in biggest scc: %d"
    ...
    print(
        "# nodes in graph: %d, # of edges in graph: %d, \
       percentage nodes, edges in scc: (%0.4f,%0.4f), percentage nodes, \
       edges in non-scc: (%0.4f,%0.4f)"
        % (
            num_of_nodes,
            num_of_edges,
            len(scc_nodes) * 1.0 / num_of_nodes,
            len(scc_edges) * 1.0 / num_of_edges,
            len(nonscc_nodes) * 1.0 / num_of_nodes,
            len(nonscc_edges) * 1.0 / num_of_edges,
        )
    )
